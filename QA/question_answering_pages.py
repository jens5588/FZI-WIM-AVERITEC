...
def run(model, tokenizer, question_retrieval_file, mode, top_k, combined):
    ...
    output_file = f'./data/qa_generated_questions_{combined_text}_{mode}_top{top_k}_greedy_sampling_2215.jsonl'
    ...
    for element in tqdm(question_retrieval[1400:]):
        claim = element['claim']
        ...
        for pair in question_chunks_pairs:
            ...
            if combined:
                ...
                urls = element['urls'][:top_k]
                get_text_from_url(tokenizer, claim_id, urls)
                for chunk in sentence_chunks:
                    formatted_input = get_formatted_input(messages, chunk)
                    generation, token_prob = generate_response(model, tokenizer, formatted_input, terminators)
                    response = generation.replace(formatted_input, "")
...
if __name__ == '__main__':
    ...
    model_id = '/hkfs/work/workspace/scratch/vl8701-llm/Llama3-ChatQA-1.5-70B'
    ...
    # model = None
    run(model, tokenizer, './data/test/sorted_claim_generated_question_512_test.jsonl', mode='test',
