...
import io
from bs4 import BeautifulSoup
from src.retrieval.html2lines import url2lines
...
parser.add_argument('--n_to_compute', default=-1, type=int,
                    help='How many claims to work through. Useful for larger corpus.')
...
if args.resume != "":
    ...
    for line in open(args.resume, "r"):
        ...
        if claim != next_claim[
            "claim"]:  # Bit of a hack but I am intertionally causing a fencepost error here to rebuild the last claim, as we do not know if it was finished or not
...
api_key = "AIzaSyBqZuN7NanrylUXU7NamXCpzo4nL_zgRes"
search_engine_id = "478df4a217ddf44f5"
        ...
        for page_num in range(args.n_pages):
            ...
            for result in search_results:
                ...
                if link in visited:
                    ...
                    store_file_path = args.store_folder + "/search_result_" + str(index) + "_" + str(
                        store_counter) + ".store"
